...
describe('define', function () {
  const fields = [{
    ...
    length: 32,
    allowLess: true,
    default: wshUtil.zeros(32)
  }]

  it('should trim zeros', function () {
    var someOb = {}
    wshUtil.defineProperties(someOb, fields)
      // Define Properties
    someOb.r = '0x00004'
  ...
  it('shouldn\'t allow wrong size for exact size requirements', function () {
    ...
    assert.throws(function () {
      ...
      wshUtil.defineProperties(someOb, tmp)
    })
  })
  ...
  it('it should accept rlp encoded intial data', function () {
    ...
    wshUtil.defineProperties(someOb, fields, data)
    ...
    wshUtil.defineProperties(someOb2, fields, rlpEncoded)
    assert.equal(someOb2.serialize().toString('hex'), rlpEncoded, 'the constuctor should accept rlp encoded buffers')

    var someOb3 = {}
    wshUtil.defineProperties(someOb3, fields, expectedArray)
  ...
  it('it should not accept invalid values in the constuctor', function () {
    var someOb = {}
    assert.throws(function () {
      wshUtil.defineProperties(someOb, fields, 5)
    }, 'should throw on nonsensical data')

    assert.throws(function () {
      wshUtil.defineProperties(someOb, fields, Array(6))
